...
public class UIBtnEvent2 : MonoBehaviour, 
...
IPointerClickHandler {
	...
	void Start () {
		...
		gameObject.GetComponent<RectTransform> ().position = new Vector3 (Screen.width / 2 + 10, Screen.height / 2 + 165, 0);
	}
	
	// Update is called once per frame
	void Update () {
		UIText.GetComponent<Text>().text = Game_MainScript.btn_2;
	}
	...
	public void OnPointerClick(PointerEventData eventData)
	{
		if(Game_MainScript.DontClick == false)
		{
			...
			Game_MainScript.BtnTwoClick = true;
		}
	}
	...
	{
		SoundAnswer.Btn02 = false;
		Game_MainScript.TestSound.GetComponent<SoundAnswer> ().enabled = false;
		
		if(SoundAnswer.Btn01 == false && SoundAnswer.Btn03 == false)
		{
			InBtnTime = 0.0f;
		}
	}
